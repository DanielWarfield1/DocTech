<script>
    ...
    export const wordsAfterKeyword = () =>
        startIndex ? words.slice(startIndex) : [];
...
<section class={main ? "main" : ""}>
    ...
    <button
        class={started ? (startIndex ? "listening" : "waiting") : "stopped"}
        on:click={() => {
            if (started) return;
            started = true;
            recognition.start();
        ...
        >{started
            ? startIndex
                ? "Listening..."
                : '"Hey DocTech, ..."'
            : "Click to start listening"}</button
    >
</section>
...
<style>
    ...
    .listening {
        ...
        animation: 1s cubic-bezier(0.07, -0.02, 1, 0.86) infinite alternate
            pulsing;
    }
